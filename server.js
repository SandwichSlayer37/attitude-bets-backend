...
// -- IMPROVED Reddit Search Logic --
async function getRedditSentiment(homeTeam, awayTeam) {
    const key = `reddit_${homeTeam}_${awayTeam}`;
    return fetchData(key, async () => {
        try {
            // Broader search for either team name OR common aliases
            const homeAliases = homeTeam.split(' ').pop(); // e.g., "Jays" from "Toronto Blue Jays"
            const awayAliases = awayTeam.split(' ').pop(); // e.g., "Twins" from "Minnesota Twins"
            const searchQuery = `(${homeTeam} OR ${awayTeam} OR ${homeAliases} OR ${awayAliases})`;
            
            const searchResults = await r.getSubreddit('sportsbook').search({ query: searchQuery, sort: 'hot', time: 'day', limit: 50 });
            ...
            searchResults.forEach(item => {
                ...
                const hasHome = content.includes(homeTeam.toLowerCase()) || content.includes(homeAliases.toLowerCase());
                const hasAway = content.includes(awayTeam.toLowerCase()) || content.includes(awayAliases.toLowerCase());
                
                // Only count posts/comments that mention BOTH teams to ensure relevance
                if (hasHome && hasAway) {
                    relevantPosts++;
                    // A simple way to score: if a team is mentioned more, it gets more points
                    const homeCount = (content.match(new RegExp(homeTeam.toLowerCase(), 'g')) || []).length + (content.match(new RegExp(homeAliases.toLowerCase(), 'g')) || []).length;
                    const awayCount = (content.match(new RegExp(awayTeam.toLowerCase(), 'g')) || []).length + (content.match(new RegExp(awayAliases.toLowerCase(), 'g')) || []).length;
                    if (homeCount > awayCount) homeScore++;
                    if (awayCount > homeCount) awayScore++;
                }
            });
            
            if (relevantPosts === 0) return { home: 5.0, away: 5.0 }; // Return neutral if no relevant posts found
