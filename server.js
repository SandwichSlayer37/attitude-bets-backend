...
async function getOdds(sportKey) {
    ...
    try {
        ...
        console.log(`Response from Odds API for ${sportKey}:`, JSON.stringify(response.data, null, 2));

        return response.data;
    } catch (error) {
        console.error(`Error fetching odds for ${sportKey}:`, error.response ? error.response.data : error.message);
        return [];
    }
}
