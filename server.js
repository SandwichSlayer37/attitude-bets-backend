...
async function getOdds(sportKey) { return fetchData(`odds_${sportKey}`, async () => { try { const { data } = await axios.get(`https://api.the-odds-api.com/v4/sports/${sportKey}/odds/?regions=us&markets=h2h,spreads,totals&oddsFormat=decimal&apiKey=${ODDS_API_KEY}`); return data; } catch (error) { return []; } }, 900000); }
...
async function getWeatherData(teamName) { const location = teamLocationMap[teamName]; if (!location) return null; const { lat, lon } = location; return fetchData(`weather_${lat}_${lon}`, async () => { try { const { data } = await axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,precipitation,wind_speed_10m&wind_speed_unit=kmh`); return { temp: data.current.temperature_2m, wind: data.current.wind_speed_10m, precip: data.current.precipitation }; } catch (e) { return null; } }); }

// -- NEW & IMPROVED Reddit Search Logic --
async function getRedditSentiment(homeTeam, awayTeam) {
    ...
    return fetchData(key, async () => {
        try {
            ...
            // Perform two separate searches in parallel
            ...
            // The score is the number of posts/comments found for each team
            const homeScore = homeResults.length;
            const awayScore = awayResults.length;

            const totalScore = homeScore + awayScore;

            // If there's no chatter for either team, return neutral
            if (totalScore === 0) {
            ...
            return { home: 5, away: 5 }; // Return neutral on error
        }
    }, 1800000);
}
...
const fullEngineCode = {
    ...
    runPredictionEngine: function(game, sportKey, context) {
        const { teamStats, weather, redditSentiment } = context;
        const weights = this.getDynamicWeights(sportKey);
    ...
    appGetPredictions: async function(req, res) {
        ...
        try {
            ...
            const predictions = await Promise.all(games.map(async (game) => {
                ...
                const predictionData = this.runPredictionEngine(game, sport, context);
...
// Replace the placeholder functions with the full versions
getDynamicWeights = fullEngineCode.getDynamicWeights;
runPredictionEngine = fullEngineCode.runPredictionEngine;
app.get('/predictions', (req, res) => fullEngineCode.appGetPredictions(req, res));
