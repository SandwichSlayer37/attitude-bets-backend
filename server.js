...
const teamLocationMap = {
    // MLB
    'Toronto Blue Jays': { lat: 43.64, lon: -79.38 },
    'Boston Red Sox': { lat: 42.34, lon: -71.09 },
    'New York Yankees': { lat: 40.82, lon: -73.92 },
    'Tampa Bay Rays': { lat: 27.76, lon: -82.65 },
    'Baltimore Orioles': { lat: 39.28, lon: -76.62 },
    // NHL
    'Vancouver Canucks': { lat: 49.27, lon: -123.12 },
    'Edmonton Oilers': { lat: 53.54, lon: -113.49 },
    'Calgary Flames': { lat: 51.04, lon: -114.07 },
    'Seattle Kraken': { lat: 47.62, lon: -122.35 },
    // NFL
    'Kansas City Chiefs': { lat: 39.04, lon: -94.48 },
    'Buffalo Bills': { lat: 42.77, lon: -78.78 },
    /* Note: For full functionality, you'll need to add coordinates for all teams. */
};
...
async function getRedditSentiment(homeTeam, awayTeam) {
    ...
    return fetchData(key, async () => {
        try {
            ...
            // -- FIX: Safely check for post titles and comment bodies to prevent crashes --
            searchResults.forEach(item => {
                const content = (item.title || '') + ' ' + (item.selftext || item.body || '');
                const lowerCaseContent = content.toLowerCase();
                
                if (lowerCaseContent.includes(homeTeam.toLowerCase())) homeScore++;
                if (lowerCaseContent.includes(awayTeam.toLowerCase())) awayScore++;
            });
...
function runPredictionEngine(game, sportKey, context) {
    ...
    if (confidence > 7.5) { // Confidence is > 7.5% from 50/50
    ...
    if (weather && sportKey !== 'icehockey_nhl') {
        const weatherImpact = (50 - weather.wind) * 0.1 + (50 - Math.abs(21 - weather.temp)) * 0.05;
        rawPower += (weatherImpact - 50) * (weights.weather / 10);
        factors['Weather'] = { value: (weatherImpact - 50) * (weights.weather / 10), homeStat: `${weather.temp}°C`, awayStat: `${weather.wind.toFixed(1)} km/h` };
    }
...
// --- API ENDPOINTS ---
app.get('/predictions', async (req, res) => { /* ... Unchanged ... */ });
...
// Helper functions for unchanged parts
...
app.get('/predictions', async (req, res) => { const { sport } = req.query; if (!sport) return res.status(400).json({ error: "Sport parameter is required." }); try { const [games, teamStats] = await Promise.all([ getOdds(sport), getTeamStats(sport) ]); if (!games || games.length === 0) { return res.json({ message: `No upcoming games for ${sport}. The season may be over.` }); } const predictions = await Promise.all(games.map(async (game) => { const [weather, redditSentiment] = await Promise.all([ getWeatherData(game.home_team), getRedditSentiment(game.home_team, game.away_team) ]); const context = { teamStats, weather, redditSentiment }; const predictionData = runPredictionEngine(game, sport, context); return { game, prediction: predictionData }; })); res.json(predictions.filter(p => p && p.prediction)); } catch (error) { console.error("Prediction Error:", error); res.status(500).json({ error: "Failed to process predictions.", details: error.message }); } });
